...
typedef enum source_connect_detect_status_e {
    SOURCE_PLUG_OUT = 0,
    SOURCE_PLUG_IN = 1,
} source_connect_detect_status_t;

class CTvDevicesPollDetect {
public:
    CTvDevicesPollDetect();
    ~CTvDevicesPollDetect();
    int startDetect();
...
private:
    static void *TvDevicesPollDetectThread(void* data);
